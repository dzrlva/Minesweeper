...
class Board:
    ...
    def __init__(self, app, size, width, height):
        ...
        self.board = {
            Point(row, col): dict() for row, col in Point.range([self.rows, self.cols])
        }
        self.__createBoard()
        ...
        for col in range(self.cols):
            ...
            for row in range(self.rows):
                ...
                self.board[Point(row, col)]["hex"] = Hexagon(
                    ...
                    hexY,
                    self.hexLength,
                    COLORS["hexagon.inactive"],
                    COLORS["hexagon.outline"],
        ...
        if text == "BOMB":
            ...
            if "flag" in cell:
                ...
                text = self.app.canvas.create_text(
                    ...
                    cell["hex"].center.y,
                    anchor="c",
                    fill=COLORS["hexagon.text"],
                    text=text,
                    state="disabled",
                    font=(self.app.font[0], 13),
                )
                cell["text"] = text
        return True
